...
urbaClicUtils.urlify = function (text) {
    ...
    closest: function (map, layer, latlng, vertices) {
        ...
        var i, n, distance, latlngs = layer.getLatLngs(),
            mindist = 1 / 0,
            result = null;
        if (vertices) {
            for (i = 0, n = latlngs.length; n > i; i++) {
                var ll = latlngs[i];
                distance = urbaClicUtils.closestF.distance(map, latlng, ll), mindist > distance && (mindist = distance, result = ll, result.distance = distance)
            }
            return result
        }
        for (layer instanceof L.Polygon && latlngs.push(latlngs[0]), i = 0, n = latlngs.length; n - 1 > i; i++) {
            var latlngA = latlngs[i],
                latlngB = latlngs[i + 1];
            distance = urbaClicUtils.closestF.distanceSegment(map, latlng, latlngA, latlngB), mindist >= distance && (mindist = distance, result = urbaClicUtils.closestF.closestOnSegment(map, latlng, latlngA, latlngB), result.distance = distance)
        }
        return result
    },
    closestLayer: function (map, layers, latlng) {
        for (var mindist = 1 / 0, result = null, ll = null, distance = 1 / 0, i = 0, n = layers.length; n > i; i++) {
            var layer = layers[i];
            if ("function" == typeof layer.getLatLng) ll = layer.getLatLng(), distance = urbaClicUtils.closestF.distance(map, latlng, ll);
            else if ("function" == typeof layer.getLayers)
                for (var mindist2 = 1 / 0, layers2 = layer.getLayers(), i2 = 0, n2 = layers2.length; n2 > i2; i2++) {
                    var layer2 = layers2[i2];
                    ll = urbaClicUtils.closestF.closest(map, layer2, latlng), ll && ll.distance < mindist2 && (distance = ll.distance, mindist2 = distance)
    ...
    urbaClic = function (obj, options) {
        var container = obj,
            ...
            cadastre_query = null,
            zoom_timeout = null,
            focusOff_timeout = null,
            loadParcelle_timeout = null,
            autocomplete_pos = -1,
            autocomplete_open = !1,
            current_parcelle = {
                loadings: []
        ...
        var input = jQuery("#urbaclic-search"),
            ban_options = autocomplete_params,
        ...
        var autocomplete_press = function (val) {
                var ul = jQuery("#urbaclic-autocomplete ul"),
                    updateStyle = function () {
                        return ul.find("a.focus").removeClass("focus"), 0 > autocomplete_pos ? autocomplete_hide() : (autocomplete_pos >= 0 && jQuery(ul.find("a")[autocomplete_pos]).addClass("focus"), void ul.animate({
                ...
                autocomplete_open ? ("Esc" == val && autocomplete_hide(), "Enter" == val && initMarker(jQuery(ul.find("a")[autocomplete_pos]).data(), !0), "Down" == val && autocomplete_pos < ul.find("a").length - 1 && autocomplete_pos++, "Up" == val && autocomplete_pos--, "Down" != val && "Up" != val || updateStyle()) : "Down" == val && (autocomplete_show(), autocomplete_pos = 0, updateStyle())
            },
            ...
            autocomplete_hide = function () {
                autocomplete_open = !1, autocomplete_pos = -1, jQuery("#urbaclic-autocomplete ul a.focus").removeClass("focus"), clearTimeout(focusOff_timeout), focusOff_timeout = setTimeout(function () {
            ...
            autocomplete = function (loadFirst) {
                ...
                if (0 == t.search(/\d{1,2}(\.\d+)?,\s*\d{1,2}(\.\d+)/)) {
                    ...
                    var latlng = t.split(",");
                    ul.length || (ul = jQuery('<ul class="urbaclic-autocomplete"></ul>').insertAfter(input).hide(), ul.css("top", input.outerHeight() - 2));
                    var data = {
                        query: "",
                    ...
                    return ul.html(Templates.autocomplete(data)).slideDown(), loadFirst === !0 && initMarker(jQuery("#urbaclic-autocomplete ul a").first().data()), !1
                }
                if (t.length > 1) {
                    ul.length || (ul = jQuery('<ul class="urbaclic-autocomplete"></ul>').insertAfter(input).hide(), ul.css("top", input.outerHeight() - 2));
                    ...
                    params.q = t, ban_query = jQuery.getJSON(url, params, function (data) {
                        ban_query = null, data.features.length ? (ul.html(Templates.autocomplete(data)).slideDown(), loadFirst === !0 && initMarker(jQuery("#urbaclic-autocomplete ul a").first().data())) : ul.html("").fadeOut()
                    })
            ...
            initMap = function () {
                if (L.TileLayer.WMTS = L.TileLayer.extend({
                    ...
                    getTileUrl: function (tilePoint, zoom) {
                        var map = this._map;
                        return crs = map.options.crs, tileSize = this.options.tileSize, nwPoint = tilePoint.multiplyBy(tileSize), nwPoint.x += 1, nwPoint.y -= 1, sePoint = nwPoint.add(new L.Point(tileSize, tileSize)), nw = crs.project(map.unproject(nwPoint, zoom)), se = crs.project(map.unproject(sePoint, zoom)), tilewidth = se.x - nw.x, zoom = map.getZoom(), ident = this.matrixIds[zoom].identifier, X0 = this.matrixIds[zoom].topLeftCorner.lng, Y0 = this.matrixIds[zoom].topLeftCorner.lat, tilecol = Math.floor((nw.x - X0) / tilewidth), tilerow = -Math.floor((nw.y - Y0) / tilewidth), url = L.Util.template(this._url, {
                    ...
                    jQuery(".urbaclic-map").length || jQuery('<div class="urbaclic-map"></div>').appendTo(container), map = L.map(jQuery(".urbaclic-map")[0], urbaClic_options.leaflet_map_options).setView([46.6795944656402, 2.197265625], 4), map.attributionControl.setPrefix(""), map.layerController = L.control.layers([], []).addTo(map);
                    var first = !0;
                    for (var i in urbaClic_options.background_layers) {
                        ...
                        if ("string" == typeof bl) {
                            ...
                            if (l) _urbaclic.addBackground(l.title, l.layer, 0 == i), first && (l.layer.addTo(map), first = !1);
                            else try {
                                bl = eval(bl)
                            } catch (err) {
        ...
        var pushHistory = function () {
            ...
            initMarker = function (params, push) {
                null == map && initMap(), input.val(params.feature.properties.label), push && pushHistory(), current_citycode = params.feature.properties.citycode, layers.marqueur && map.removeLayer(layers.marqueur);
                ({
                ...
                layers.marqueur = addMarqueurLayer(adresse_json), map.fitBounds(L.featureGroup([layers.marqueur]).getBounds()), loadParcelle(!1, push), layers.marqueur.on("dragend", function (e) {
            ...
            loadParcelle = function (fromDrag, push) {
                ...
                if (null != layers.servitudes && (map.removeLayer(layers.servitudes), layers.servitudes = null), 1 == fromDrag) {
                ...
                cadastre_query = jQuery.getJSON(url, qparams, function (data) {
                    ...
                    } else console.info("aucune parcelle trouvée à " + marker_pos.latlng.toString()), loadClosest(marker_pos.latlng, current_citycode)
                })
            },
            loadClosest = function (latlng, citycode) {
                ...
                var delta = 5e-4,
                    ...
                    limit_geojson = L.rectangle(bb).toGeoJSON(),
                    url = Cadastre_API + "cadastre/geometrie",
                    qparams = (L.rectangle(map.getBounds()), {
                ...
                cadastre_query = jQuery.getJSON(url, qparams, function (data) {
                    if (data.features.length) {
                        var layer = L.geoJson(data, {
                            ...
                            closest = urbaClicUtils.closestF.closestLayer(map, layer.getLayers(), latlng),
                            parcelle = closest.layer;
                        if (parcelle) {
                            layers.parcelle && map.removeLayer(layers.parcelle), layers.parcelle = parcelle, parcelle.addTo(map), map.fitBounds(layers.parcelle.getBounds());
                            var marker_pos = {
            ...
            getServitudesDetail = function () {
                ...
                jQuery.each(backgroundLayers, function (t, l) {
                    ...
                    null == layers.servitudes && (layers.servitudes = L.layerGroup(), layers.servitudes.addTo(map), updateLayerController());
                    var map_container = jQuery(this),
                        servitude_id = map_container.data("servitudeid"),
                        properties = map_container.data("properties"),
                        options = jQuery.extend(urbaClic_options.leaflet_map_options, {
                            zoomControl: !1
                        }),
                        url = URBA_API + "servitudes/" + servitude_id,
            ...
            showData = function (feature, layer, evt) {
                ...
                if (jQuery(".urbaclic-data").html(Templates.parcelleData(current_parcelle.data)), urbaClic_options.getadresse) {
                    ...
                    current_parcelle.loadings.ban_query = jQuery.getJSON(url, params, function (data) {
                        addBanLayer(data), void 0 != data.features[0] && (current_parcelle.data.adresse = data.features[0].properties, jQuery(".urbaclic-data").html(Templates.parcelleData(current_parcelle.data)))
                    })
                }
                if (urbaClic_options.getservitude) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            current_parcelle.data.servitudes = data, jQuery(".urbaclic-data").html(Templates.parcelleData(current_parcelle.data)), getServitudesDetail()
                        }
                    })
                }
        ...
        return initial_url.split("#").length > 1 ? (initial_url = initial_url.split("#")[0], loadFromUrl()) : autocomplete(), input.keydown(function (e) {
            var c = e.keyCode;
            ...
            autocomplete_hide()
        }), _urbaclic.map = map, _urbaclic.loadParcelle = loadParcelle, _urbaclic.initMarker = initMarker, _urbaclic
    };
    var BAN_API = "https://api-adresse.data.gouv.fr/",
        ...
        checklibs = function () {
            ...
            for (var i in dependences) "undefined" == typeof window[i] && (0 == jQuery('script[src="' + dependences[i] + '"]').length && jQuery('<script src="' + dependences[i] + '"></script>').appendTo("body"), ready = !1);
        ...
        start = function () {
            ...
            _urbaclic.lang = lang = "fr", i18n.init({
                ...
                return text = text.replace(/\[\*\* /g, "<mark>").replace(/ \*\*\]/g, "</mark>"), new Handlebars.SafeString(text)
                ...
                if (1 == nPage) return "";
                for (var i = 1; nPage >= i; ++i) res += "<li" + (i == n ? ' class="active"' : "") + ">", res += '<a href="#" data-page=' + i + ">" + i + "</a></li>";
                ...
                return "???" == res && (value = value.charAt(0).toLowerCase() + value.slice(1), res = i18n.t(value, options.hash), res = res.charAt(0).toUpperCase() + res.slice(1)), "???" == res && (value = value.charAt(0).toUpperCase() + value.slice(1), res = i18n.t(value, options.hash), res = res.charAt(0).toLowerCase() + res.slice(1)), "???" == res ? (console.warn('i18n "' + value + '" NOT FOUND'), value) : res
                ...
                if (value) {
                    ...
                    var text, ellipsis;
                    return value.indexOf("<!--- excerpt -->") && (value = value.split(EXCERPT_TOKEN, 1)[0]), ellipsis = value.length >= length ? "..." : "", text = marked(value.substring(0, length) + ellipsis), text = text.replace("<a ", "<span ").replace("</a>", "</span>"), new Handlebars.SafeString(text)
                }
            ...
            for (var tmpl in Templates) {
                ...
                t.length ? (Templates[tmpl] = t.html(), console.info("loaded.")) : console.info("not found, use default template."), "string" != typeof Templates[tmpl] && (Templates[tmpl] = Templates[tmpl].join("\n")), Templates[tmpl] = Handlebars.compile(Templates[tmpl])
            }
            container = jQuery("#urbaclic"), container.length && (window.urbaClic_autoload = [], container.each(function () {
                var obj = jQuery(this);
